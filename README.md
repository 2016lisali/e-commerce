...
#### Method 2
  ...
  Use the `create-payload-app` CLI to clone this template directly to your machine:ß
...
1. `yarn && yarn dev` to install dependencies and start the dev server. `npm install --legacy-peer-deps` install some older versions of dependencies``≠≠
...
# Add a field to existing schema. Take add image field to Category for example
- payload-types.ts, find export interface Category {
  media?: string | Media | null;
}
- collections/Categories.ts, add { name: 'media', type: 'upload', relationTo: 'media' }, to fields array
- 'yarn payload generate:graphQLSchema': generate payload schema (Every time we modify schema, we need to run the following script)
- 'yarn generate:types'
